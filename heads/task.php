    ...
    <section class="Dashboard fw-bold text-center">
        <div class="users_info pt-4 bg-dark">
            ...
            <table class="table table-dark table-striped">
                        ...
                        <th>
                            <div class="container">
                                <div class="row methods mt-3 mb-4 rounded">
                                    ...
                                    <a class="col nav-link p-2 active" href="./task.php" >tasks</a>
                ...
                <tbody>
                    <tr>
                        <td colspan="3"><button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo">add new task</button></td>
                        <?php
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['addcommitte'])) {
        ...
        if (empty($formErrors)) {
                ...
                $insert = $con->prepare("INSERT INTO 
                            tasks( title, description, path, head_id)
                ...
                if ($insert) {
                    ...
                    <strong>the task was added</strong>  
    ...
    if(isset($_POST['deletecommitte'])){
        $id = $_POST['id'];
        $selec = $con->prepare(' select path from tasks where id = ? ');
        $selec->execute(array($id));
        $committ = $selec->fetch();
        $p = 'uploads/';
        unlink($p.$committ[0]);
        $select2 = $con->prepare("delete from tasks where id = ? ");
        $select2->execute(array($id));
        if($select2){
            echo '<div class="alert alert-success alert-dismissible fade show" style=" width: 90%;" role="alert">
            <strong>the task was deleted</strong>  
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
        }
    }
    ...
    if(isset($_POST['editproduct'])){
        ...
        if(empty($erro)){
            ...
            if ($aa) {
                echo '<div class="alert alert-success alert-dismissible fade show" style=" width: 90%;" role="alert">
                <strong>the task was updated</strong>  
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>';
            }
        }
...
?>
        ...
        <table class="table table-dark table-striped mt-1">
            ...
            <tbody>
                <?php
                $select = $con->prepare(' select * from tasks where head_id = ? ');
            ...
            ?>
                ...
                <td><?php
                    if($committe['path']){
                        echo '<a href="uploads/'.$committe['path'].'" target="_blank"><div color="black"><button class="btn btn-primary" >open task</button></div></a><br>';
                    }
                    ?></td>
                    <td><?=$committe['created']?></td>
                    <td><button class="btn btn-danger"  onclick="deletecommitte(<?=$committe['id']?>)">delete</button>
                    <button class="btn btn-secondary" onclick="editcommitte(`<?=$committe['title']?>` ,`<?=$committe['description']?>`,`<?=$committe['id']?>`)">update</button>
                    </td>
                </tr>
                <?php } ?>
            </tbody>
